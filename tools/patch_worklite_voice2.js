...
function ensureReplace(search, replace) {
  if (!text.includes(search)) throw new Error('Missing marker: ' + search);
  text = text.replace(search, replace);
}
ensureReplace('import type { Mode, QualityTier } from "../../shared/contracts/index";', 'import type { Mode, QualityTier, VoiceSettings } from "../../shared/contracts/index";');
ensureReplace('let renderStatusText = "Renderer idle";', `let renderStatusText = "Renderer idle";
const speechRuntime: {
...
ensureReplace('<button id="apply-client-state" type="button">Apply State</button>', `<button id="apply-client-state" type="button">Apply State</button>
...
ensureReplace('  applyButton.addEventListener("click", () => {\n    applyViewportValues(false);\n  });', `  applyButton.addEventListener("click", () => {
    applyViewportValues(false);
  });
...
ensureReplace('  let orientationSummary = "orientation pending";', '  let orientationSummary = "orientation pending";\n  let talkStatusSummary = "speech idle";');
ensureReplace('  let baseSkeletonRoot: any | undefined;', `  let baseSkeletonRoot: any | undefined;
  let jawBone: any | undefined;
  let headBone: any | undefined;
  let neckBone: any | undefined;
  let talkAmount = 0;
  let talkClock = 0;`);
ensureReplace('    rebindEntityRenderRootBone(baseAvatarEntity, baseSkeletonRoot);', `    rebindEntityRenderRootBone(baseAvatarEntity, baseSkeletonRoot);
...
ensureReplace('  app.on("update", (dt: number) => {', `  const postUpdateHandler = (dt: number) => {
    talkClock += dt;
...
ensureReplace('${latestRenderStats}. ${orientationSummary}. ${avatarBoundsSummary}. Canvas ${canvas.clientWidth}x${canvas.clientHeight}`', '${latestRenderStats}. ${orientationSummary}. ${avatarBoundsSummary}. ${talkStatusSummary}. Canvas ${canvas.clientWidth}x${canvas.clientHeight}');
ensureReplace('      window.removeEventListener("resize", resize);\n      app.destroy();', '      window.removeEventListener("resize", resize);\n      app.systems.off("postUpdate", postUpdateHandler);\n      app.destroy();');
ensureReplace('function getInputValue(id: string): string {', `function getTextAreaValue(id: string): string {
...
if (!text.includes('async function loadVoiceSettings(): Promise<VoiceSettings>')) {
text += `
...
async function speakAvatarText(text: string): Promise<void> {
  ...
  const payload = await response.json() as { mimeType?: string; audioBase64?: string; };
...
function playAudioBase64(audioBase64: string, mimeType: string): void {
  ...
  audio.addEventListener("ended", () => stopAvatarSpeech());
  audio.addEventListener("error", () => stopAvatarSpeech());
  void audioContext.resume().catch(() => undefined);
  void audio.play().catch(() => stopAvatarSpeech());
  tick();
}
...
function speakWithBrowserVoice(text: string, settings: VoiceSettings): void {
  ...
  utterance.onstart = () => { speechRuntime.speaking = true; };
  utterance.onboundary = () => { speechRuntime.talkLevel = 0.9; };
