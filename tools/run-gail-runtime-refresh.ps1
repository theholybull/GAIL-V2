param(
  ...
  [string]$BlenderExe = "",
  [string]$RunLabel = ("gail-runtime-refresh-" + (Get-Date -Format "yyyyMMdd-HHmmss"))
)
...
$ReportPath = Join-Path $RunRoot "gail-runtime-report.json"
...
if ($Process.ExitCode -ne 0) {
  throw "Gail runtime export failed. See $LogPath"
}

if (-not (Test-Path -LiteralPath $ReportPath -PathType Leaf)) {
  throw "Gail runtime export completed without report: $ReportPath"
}

$Targets = @(
  [ordered]@{
    source = Join-Path $ExportRoot "avatar\base_face\gail_base_avatar.glb"
    target = Join-Path $RepoRoot "playcanvas-app\assets\gail\avatar\base_face\gail_base_avatar.glb"
  },
  [ordered]@{
    source = Join-Path $ExportRoot "hair\meili_hair\meili_hair.glb"
    target = Join-Path $RepoRoot "playcanvas-app\assets\gail\hair\meili_hair\meili_hair.glb"
  },
  [ordered]@{
    source = Join-Path $ExportRoot "clothes\gail_top\gail_top.glb"
    target = Join-Path $RepoRoot "playcanvas-app\assets\gail\clothes\gail_top\gail_top.glb"
  },
  [ordered]@{
    source = Join-Path $ExportRoot "clothes\gail_pants\gail_pants.glb"
    target = Join-Path $RepoRoot "playcanvas-app\assets\gail\clothes\gail_pants\gail_pants.glb"
  },
  [ordered]@{
    source = Join-Path $ExportRoot "clothes\gail_sandals\gail_sandals.glb"
    target = Join-Path $RepoRoot "playcanvas-app\assets\gail\clothes\gail_sandals\gail_sandals.glb"
  },
  [ordered]@{
    source = Join-Path $ExportRoot "accessories\gail_bundle\gail_accessories.glb"
    target = Join-Path $RepoRoot "playcanvas-app\assets\gail\accessories\gail_bundle\gail_accessories.glb"
  },
  [ordered]@{
    source = Join-Path $ExportRoot "accessories\gail_bundle\gail_accessories.glb"
    target = Join-Path $RepoRoot "playcanvas-app\assets\gail\accessories\gail_bundle\gail_bracelets.glb"
  }
)
