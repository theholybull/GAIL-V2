...
# Vera formal wardrobe name substrings (lowercase)
BLAZER_NAME_TOKENS = ("blazer",)
BLOUSE_NAME_TOKENS = ("blouse",)
SKIRT_NAME_TOKENS = ("skirt",)
...
def is_blazer_mesh(mesh):
    name = mesh.name.lower()
    return any(token in name for token in BLAZER_NAME_TOKENS)


def is_blouse_mesh(mesh):
    name = mesh.name.lower()
    return any(token in name for token in BLOUSE_NAME_TOKENS)


def is_skirt_mesh(mesh):
    name = mesh.name.lower()
    return any(token in name for token in SKIRT_NAME_TOKENS)
...
def classify_meshes(meshes):
    ...
    for mesh in meshes:
        if is_body_mesh(mesh):
            ...
            shoes.append(mesh)
        elif is_blazer_mesh(mesh):
            blazer.append(mesh)
        elif is_blouse_mesh(mesh):
            blouse.append(mesh)
        elif is_skirt_mesh(mesh):
            skirt.append(mesh)
        else:
            # fallback — include in blazer group so nothing is silently dropped
            blazer.append(mesh)
...
def main():
    ...
    targets = {
        "avatar":  output_root / "avatar" / "base_face" / "vera_base_avatar.glb",
        "hair":    output_root / "hair" / "vera_hair.glb",
        "blazer":  output_root / "clothing" / "vera_blazer.glb",
        "blouse":  output_root / "clothing" / "vera_camisole.glb",
        "skirt":   output_root / "clothing" / "vera_jeans.glb",
        "shoes":   output_root / "clothing" / "vera_heels.glb",
    }
    ...
    report["exports"]["blazer"] = export_group("blazer", targets["blazer"], armature, groups["blazer"], export_morph=False)
    report["exports"]["blouse"] = export_group("blouse", targets["blouse"], armature, groups["blouse"], export_morph=False)
    report["exports"]["skirt"]  = export_group("skirt",  targets["skirt"],  armature, groups["skirt"],  export_morph=False)
