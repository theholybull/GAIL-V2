...
class Quaternions:
    ...
    def rotate_vector(self, vectors):
        ...
        """
        # 将向量转换为纯四元数（实部为0）
        vector_quats = np.concatenate([np.zeros(vectors.shape[:-1] + (1,)), vectors], axis=-1)  # (..., 4)

        # 执行四元数旋转：v' = q * v * q_conj
        rotated_quats = self * Quaternions(vector_quats) * (-self)

        # 返回旋转后的向量部分（虚部）
        return rotated_quats.qs[..., 1:]
