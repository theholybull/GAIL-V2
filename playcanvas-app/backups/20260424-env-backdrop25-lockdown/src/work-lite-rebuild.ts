...
const CLIENT_ASSET_VERSION_SALT = "20260424-env-backdrop25";
...
function wireUi(root: HTMLElement): void {
  ...
  if ("speechSynthesis" in window) {
    window.speechSynthesis.onvoiceschanged = () => populateVoiceSelect(voiceSelect);
  }
...
async function bootStage(root: HTMLElement): Promise<void> {
  ...
  const updateAvatarMotionSummary = () => {
    ...
    state.avatarMotionSummary = `${modeText} | voice ${speechState.engineLabel} ${speechState.speaking ? "speaking" : "idle"} | clips idle:${clipStatus.idle} talk:${clipStatus.talk} listen:${clipStatus.listen} ack:${clipStatus.ack} | morphs ${speechMorphTargets.length}/${eyeMorphTargets.length} lids ${eyelidRigNodes.length}${skippedModulesText}`;
    syncShellStatePanel();
    syncAvatarMotionOverlay();
  };
...
async function switchToPersona(persona: string): Promise<void> {
  ...
  try {
    ...
    // 6. Reboot the stage with new persona
    state.sceneReady = false;
    await bootStage(root);

    // 7. Refresh the visible persona indicator
    syncPersonaSelectorUi();
  } catch (err) {
...
function pickDanceCandidates(maxCount = 12): string[] {
  const pool = [...DANCE_ANIMATION_FILES];
  if (lastWorkingDanceFile) {
    const idx = pool.indexOf(lastWorkingDanceFile);
    if (idx >= 0) {
      pool.splice(idx, 1);
      pool.unshift(lastWorkingDanceFile);
    }
  }
  ...
  if (lastWorkingDanceFile && !pool.includes(lastWorkingDanceFile)) {
    pool.unshift(lastWorkingDanceFile);
  }
...
async function handleDanceCommand(): Promise<boolean> {
  ...
  const selectedDanceState = (["dance", "ack", "talk", "listen"] as AvatarAnimationState[])
  ...
  try {
    ...
    animComponent.speed = DANCE_PLAYBACK_SPEED;
...
function syncTimeOfDayLighting(runtime?: StageRuntime): void {
  ...
  const lightScale = mode === "night" ? (NIGHT_MODE_LIGHT_LEVEL / 100) : sliderLightScale;
